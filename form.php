...
elseif(!preg_match("/^[A-Za-z ]*$/",$_POST["pname"]))
{
 ...
 if(empty($_POST["pemail"])) {
  ...
  } elseif(!preg_match("/^[a-z0-9]+([_\\.-][a-z0-9]+)*@([a-z0-9]+([\.-][a-z0-9]+)*)+\\.[a-z]{2,}$/i",$_POST["pemail"]))
   {
...
if (empty($_POST["pgender"])) {  
	...
	elseif(!preg_match('/^[0-9]{10}+$/',$_POST["pphone"])){
...
elseif(!preg_match("/^[A-Za-z ]*$/",$_POST["name"]))
{
 ...
 if(empty($_POST["email"])) {
  ...
  return $Error;
	...
	$email=$_POST['email'];
	$phone=$_POST['phone'];
	$sql="INSERT INTO students(class,name,email,phone,pname,pemail,pgender,pphone) VALUES('$class','$name','$email','$phone','$pname','$pemail','$pgender','$pphone')";
	$result=mysqli_query($conn,$sql);
	if($result){
		echo "<p style='color:red;'>it has inserted value in Student table.";
	}else{
		echo "it has problem in inserting value in Student table.</p>".mysqli_error($conn);
	}
	mysqli_close($conn);
	}
?>
...
<input type="tel"  placeholder="Enter phone" name="phone" id="phone" required>
    ...
    <input type="text" placeholder="Enter name" name="pname" id="name" required>
          
    <label for="email"><b>Email</b></label>
    <input type="text" placeholder="Enter Email" name="pemail" id="email" required>
...
<input type="tel"  placeholder="Enter phone" name="pphone" id="phone" required>
